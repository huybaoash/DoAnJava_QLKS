...
public class DatPhong {
    ...
    public void ShowRoomAvailable(){
        if (phieuDatPhongsList == null) System.out.println("Không có gì cả");
        else System.out.println(phieuDatPhongsList.size());
    ...
    void RoomDetails(PhieuDatPhong pdp,String tenphong) throws IOException {
        ...
        controller.setRoom(pdp);controller.setNvphutrach(nvphutrach);
        ...
        if (controller.getFlag() == 0) return;
        if (controller.getFlag() == 1) room = controller.getRoom();
    }
