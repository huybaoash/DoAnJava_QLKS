...
public class QLDP {
    ...
    public void Init() throws SQLException, ClassNotFoundException {
        ...
        while (db.rs.next()){
            if (phongdat.equals(db.rs.getString(2)) ){
                ...
                if (db.rs.getString("MaLP").equals("DON")) rbDon.setSelected(true);
                if (db.rs.getString("MaLP").equals("DOI")) rbDoi.setSelected(true);
                if (db.rs.getString("MaLP").equals("MANY")) rbMany.setSelected(true);
                break;
    ...
    void PressOK(ActionEvent event) throws SQLException {
        if (dtpNgayTra.getValue().isBefore(dtpNgayDat.getValue()) && dtpNgayTra.getValue().equals(dtpNgayDat.getValue())){
            JOptionPane.showMessageDialog(null,"Không thể trả trước hoặc ngay trong ngày đặt");
            dtpNgayDat.setValue(LocalDate.parse(LocalDate.now().format(formatter)));return;
        }

        String GioiTinh = "";
        if (rbNu.isSelected()) GioiTinh = "Nữ";
        else GioiTinh = "Nam";

        Khach khachDP = new Khach(txtQName.getText(),txtQPhoneNumber.getText(),txtQEmail.getText(),txtQCMND.getText(),txtQCountry.getText()
                                    ,GioiTinh, Date.valueOf(dtpQWasBorn.getValue()));
        ...
        while (db.rs.next()){
            ...
            {
                System.out.println("Cùng một người");dem++;
            }

        }

        if(dem==0) db.addKhachDB(txtQName.getText(),Date.valueOf(dtpQWasBorn.getValue().toString()),txtQPhoneNumber.getText(),txtQEmail.getText(),
        ...
        int MaPDP = 0;
        db.rs = db.stmt.executeQuery("Select COUNT(MaPhieuDP) as [Tổng] from [QLKS].[dbo].[PhieuDatPhong] ");
        while (db.rs.next()){
        ...
        String tenLP = "";
        if (rbDoi.isSelected()) tenLP = rbDoi.getText();
        if (rbDon.isSelected()) tenLP = rbDon.getText();
        if (rbMany.isSelected()) tenLP = rbMany.getText();

        room = new PhieuDatPhong(MaPDP+1,Integer.valueOf(txtIDRoom.getText()),txtNameRoom.getText(),tenLP,
                txtQName.getText(),Date.valueOf(dtpNgayDat.getValue()),Date.valueOf(dtpNgayTra.getValue()),txtRoomPrice.getText(),txtMoneyPaid.getText());

        JOptionPane.showMessageDialog(null,"Đặt phòng thành công !");
        db.addHD(Integer.valueOf(txtIDRoom.getText()),new BigDecimal(txtMoneyPaid.getText()),Date.valueOf(dtpNgayTra.getValue()),
                nvphutrach.getMaNV());
        flag = 1;
        primaryStage.close();
    }
    ...
    void PressReset(ActionEvent event) {
        txtQCMND.setText("");txtQCountry.setText("");
        txtQEmail.setText("");txtQName.setText("");
